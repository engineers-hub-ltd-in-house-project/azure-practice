...
Deny と Audit の使い分けは、導入の順序の問題です。いきなり Deny を適用すると、既存の業務やデプロイが突然止まります。まず Audit で現状の違反量を把握し、直す当てが付いてから Deny に切り替える。これが定石です。
...
片付ける前に、同じ定義を 3 通りに割り当てた結果をまとめます。

```mermaid
flowchart TB
  DEF["組み込みの定義 (キー認証を禁じる)"] --> A1["azp-ch20-deny-localauth (Deny)"]
  DEF --> A2["azp-ch20-audit-localauth (Audit)"]
  DEF --> A3["azp-ch20-mg-audit (Audit)"]
  A1 --> RG["azp-ch20-rg (リソースグループ)"]
  A2 --> RG
  A3 --> MG["azp-ch20-mg (管理グループ)"]
  RG -.->|"Deny のときは作成が拒否された"| COS["azp-ch20-cosmos (アカウント)"]
```

見どころは、定義が 1 つしかないことです。作らせないのか、作らせて記録するのかは効果の指定で決まり、どこまで効くのかは割り当てのスコープで決まります。第 6 章のロール割り当てと同じ形で、定義とスコープの組み合わせが結果を決めます。管理グループへの割り当ては、配下のサブスクリプション全部に同じルールを流します。
...
1. RBAC で Contributor を絞ることと、Policy で Deny を適用することは、どちらも「作らせない」結果を生めます。2 つの仕組みが答えている問いの違いを、第 6 章と本章の言葉で説明してください
