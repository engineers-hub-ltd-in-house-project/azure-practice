...
これで、このストレージには「誰であるか」を示さない限り誰も入れなくなりました。すべてのアクセスが ID に紐づき、ログに残り、RBAC で範囲を制御できます。信用できるネットワークや秘密の文字列を前提にせず、毎回のアクセスで ID を検証する。この設計方針は一般にゼロトラスト（Zero Trust）と呼ばれます。
