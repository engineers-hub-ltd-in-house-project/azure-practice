...
第2部はここで終了します。手順の実体は `scripts/chapters/ch09-identity.sh` にあり、本章の手順と出力はすべて本書の検証環境で通しで実行して確認済みです。
...
スクリプトは 4 段で進みます。それぞれの段に、第2部の章がそのまま対応しています。
...
2 番目と 3 番目が対になっています。同じストレージが、キーには閉じ、ID には開いている。第2部で組み立てた世界の完成形です。
...
スコープを広く（たとえばサブスクリプションに）掛けていたら、この teardown では割り当てが残り、第3章で見た残骸になります。スコープの選択はライフサイクルの設計でもある、というのが第2部の締めくくりです。
