...
第 5〜8 章で、ID の種類、ロールとスコープ、マネージド ID、そして外向きの認証手段を分解してきました。本章はそれを 1 本のシナリオに戻します。

シナリオはこうです。これから作るアプリケーションのために、ID と保管場所を用意します。ただしアクセスキーは一度も有効にしません。

本章で第 2 部は終わりです。手順の実体は `scripts/chapters/ch09-identity.sh` にあり、本章の手順と出力はすべて本書の検証環境で通しで実行して確認済みです。
...
スクリプトは 4 段で進みます。それぞれの段に、第 2 部の章がそのまま対応しています。

### 段 1 ― 器を用意する

リソースグループを作ります。第 3 章のとおり、この章で作るものは全部ここに入れ、消すときは丸ごと消します。
...
ID を割り当てるアプリケーションはまだ存在しませんが、ユーザー割り当てマネージド ID は先に作れます（第 7 章）。ID が先にあるので、権限も先に配れます。アプリの実体ができる前に、セキュリティの枠組みだけが先に完成する順序です。
...
第 8 章では、キーが有効なストレージを作ってから止めました。本章では最初から無効で作ります。キーが一度も存在しなかったのだから、漏れているキーも、ローテーションすべきキーも、この世にありません。第 4 章のハンズオンで既にこの形を使っていたことに、いまなら気づけるはずです。
...
| マネージド ID のロール    | アプリ用の権限が、ID を使うアプリができる前に配り終わっている   |
...
片付ける前に、第 2 部で分解した概念が 1 つの形に戻ったことを確かめます。

```mermaid
flowchart TB
  subgraph RG["azp-ch09-rg (リソースグループ)"]
    ID["azp-ch09-id (ユーザー割り当てマネージド ID)"]
    ST["ストレージアカウント (キー認証は無効)"]
  end
  T["テナントの台帳 (Entra ID)"] -.->|"同じ ID の 1 行"| ID
  ID -->|"Storage Blob Data Contributor (RG スコープ)"| RG
  ME["自分 (ユーザー)"] -->|"Storage Blob Data Contributor (アカウントスコープ)"| ST
```

見どころは、同じロールを 2 つの相手に、別々のスコープで割り当てたことです。アプリ用の ID にはリソースグループ全体を、自分にはストレージアカウント 1 つだけを割り当てました。範囲の広さは、その ID がこの先どこまで扱うかで決めます。ID を先に作れたので、アプリの実体ができる前にこの権限の設計が完成しています。
...
中身はリソースグループの削除 1 つだけです。それでも演習として意味があります。ロール割り当てをすべてリソースグループ配下のスコープに割り当てたので、割り当ても ID もストレージも、この 1 操作で正しい順序のまま消えます。第 6 章の「ID より先に割り当てを消す」を、設計の段階で作り込んであったわけです。

スコープを広く（たとえばサブスクリプションに）割り当てていたら、この teardown では割り当てが残り、第 3 章で見た残骸になります。スコープの選択はライフサイクルの設計でもある、というのが第 2 部の締めくくりです。
...
2. 段 2 と段 4 を入れ替えて「ストレージを先に作り、アプリをデプロイしてから権限を配る」順序にした場合、システム割り当てマネージド ID を使うとどんな制約が出ますか。第 7 章の表から説明してください
