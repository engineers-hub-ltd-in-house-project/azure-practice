...
載せ先には Azure Container Instances を使います。コンテナーを 1 つ動かすだけの単純なサービスで、秒課金のため観察用途なら数円もかかりません。読ませる対象には Azure Blob Storage を使います。ファイルを置くためのサービスで、第 15 章で正面から扱います。ここで同じ語が 2 つの意味で出てきます。Azure は、ストレージ側のファイルの入れ物も「コンテナー」と呼びます。本章では、実行するほうをコンテナー、ストレージ側を BLOB コンテナーと書き分けます。
