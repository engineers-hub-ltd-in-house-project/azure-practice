...
動かすのは最小形です。Azure のリソースを 1 つ動かし、そのリソースにストレージのファイルの一覧を取得させます。リソースの中にはキーもパスワードも置きません。

2 つの種類の違いは寿命の決まり方です。システム割り当ての ID は、割り当てたリソースと同時に作られ、同時に消えます。ユーザー割り当ての ID は、リソースとは別に作り、そのリソースが消えても残ります。この違いも実際に作って壊して確かめます。

本章では割り当てという語が 2 つの向きで出てきます。ID をリソースに割り当てること（`--assign-identity`）と、ロールを ID に割り当てること（第 6 章）です。どちらを指しているかは、割り当てる相手を見て読み分けてください。

```mermaid
flowchart LR
  subgraph SA["システム割り当て"]
    H1["ID を割り当てたリソース"] ---|"同時に作られ、同時に消える"| I1["ID"]
  end
  ...
  H1 -->|"リソースを消すと"| X1["ID も消える"]
  H2 -->|"リソースを消しても"| X2["ID は残る"]
```
...
第 6 章の演習では、サービスプリンシパルの password を控えて使い回しました。手で作るサービスプリンシパルには、この password が付いてきます。password は漏れれば不正利用され、期限が切れれば障害になり、更新には手順が要ります。ここに挙げた 3 つは、いずれも password を持つことに付いてくる問題です。
...
使いどころは、Azure のリソースが別の Azure のリソースへアクセスする場面です。プログラムと、それを動かすのに必要なファイルをひとまとめにして、そのまま実行できるようにしたものを、コンテナーと呼びます。コンテナーがストレージのファイルを読む、Web アプリがデータベースへ接続する、Functions が Key Vault からシークレットを取得する（第 16 章）。どれも、接続文字列やキーを設定値としてリソースに持たせる代わりに、リソース自身の ID でアクセスします。持たせる設定値が無いので、設定値を守る運用も要らなくなります。

マネージド ID にも制約があります。ID を割り当てられるのは Azure のリソースだけなので、手元の端末や Azure の外で動くプログラムからは使えません。マネージド ID に対応していないサービスもあります。使えるところでは password を持たずに済み、使えないところでは手で作るサービスプリンシパルに戻る、という関係です。
...
```mermaid
flowchart LR
  C["リソース (ID を割り当てた側)"] -->|"1. トークンを要求"| M["IMDS (リソース内部の問い合わせ先)"]
  M -->|"2. トークンを発行"| C
...
ID を割り当てるリソースには Azure Container Instances を使います。先ほどのコンテナーを 1 つ動かすだけの単純なサービスで、秒課金のため観察用途なら数円もかかりません。読ませる対象には Azure Blob Storage を使います。ファイルを置くためのサービスで、第 15 章で正面から扱います。ここで同じ語が 2 つの意味で出てきます。Azure は、ストレージ側のファイルの入れ物も「コンテナー」と呼びます。本章では、実行するほうをコンテナー、ストレージ側を BLOB コンテナーと書き分けます。
...
ID を割り当てるリソースより先に、ユーザー割り当てマネージド ID だけを作ります。先に作れること自体が、この種類の性質です。
...
必要なロールが並んでいるので割り当ての漏れに見えますが、割り当ては先ほど済んでいます。エラーの内容は、割り当ての漏れと伝播の待ちで同じです。見分けるには、割り当て自体があるかを確かめます。エラーが出てから別のターミナルで調べることもあるので、この手は変数を取り直すところから書いておきます。
...
これがユーザー割り当てを手で作る理由の 1 つ目です。システム割り当てには、この順序が使えません。システム割り当ての ID はリソースを作った瞬間に作られるので、リソースが無い時点では割り当てる相手が存在しないからです。

## リソースを作って読ませる

コンテナーを作ります。システム割り当てを有効にし（`[system]`）、いま作ったユーザー割り当ても同時に割り当てます。コンテナーの中では、IMDS へトークンを要求してから、そのトークンでファイルの一覧を取得します。
...
トークンが返っていません。このコンテナーには 2 つの ID が割り当てられているため、どちらとして発行するのかを Azure が決められないからです。ID を 1 つしか割り当てていないリソースでは指定は要りませんが、複数割り当てる場合は clientId で指定します。
...
本書の検証では、続くファイルの一覧の取得が 1 回目に失敗しました。出力の末尾に、先ほど自分で見たものと同じ権限のエラーが出ています。ユーザー割り当て ID への Storage Blob Data Reader も割り当て済みですが、コンテナーはその割り当てが効くより先に動き出しました。伝播遅延がもう一度出た形です。
...
テナントの台帳側を見ると、2 つの ID はどちらもサービスプリンシパルとして存在しています。システム割り当ての displayName は、ID を割り当てたリソースの名前そのものです。
...
ロールの割り当てを 1 つも作り直さずに読めました。これがユーザー割り当てを手で作る理由の 2 つ目です。リソースを作り直しても、ID とその ID に付けた権限が残るため、権限の設計をリソースの入れ替えから切り離せます。システム割り当てで同じことをすると、コンテナーを作り直すたびに新しい principalId ができ、ロール割り当てもやり直しになります。

| 観点                 | システム割り当て                              | ユーザー割り当て                       |
| -------------------- | --------------------------------------------- | -------------------------------------- |
| 作られるとき         | ID を割り当てるリソースの作成・有効化と同時   | 独立して先に作れる                     |
| 消えるとき           | そのリソースと同時                            | 自分で消すまで残る                     |
| 共有                 | リソース 1 つ専用                             | 複数のリソースに割り当てられる         |
| ロール割り当ての準備 | リソースができるまで principalId が存在しない | リソースより先にロールを割り当てられる |
...
3. 本章のコンテナーは、IMDS へ ID を指定しない 1 回目の要求ではトークンを受け取れませんでした。指定が必要だった理由を、このコンテナーに割り当てた ID の数から説明してください。ID を 1 つだけ割り当てた `azp-ch07-aci2` では指定を省けたかどうかも答えてください
