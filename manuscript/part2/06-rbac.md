...
片付ける前に、割り当てがどこに残っているかを確かめます。

```mermaid
flowchart TB
  SUB["サブスクリプション (課金の境界)"] --> A["azp-ch06-a (リソースグループ)"]
  SUB --> B["azp-ch06-b (リソースグループ)"]
  SP["azp-ch06-sp (サービスプリンシパル)"] -->|"Contributor"| A
  SP -.->|"外した Reader"| SUB
  A --> ST["ストレージアカウント (Contributor で作れた)"]
```

見どころは、同じ ID に対する割り当てを付け替えただけで、見える範囲も操作できる範囲も変わったことです。最初はサブスクリプション全体の Reader だったので、azp-ch06-b まで一覧に出ていました。いまは azp-ch06-a だけの Contributor なので、azp-ch06-b は見えず、azp-ch06-a の中では作成もできます。権限の強さは、ロールとスコープの組み合わせで決まります。
