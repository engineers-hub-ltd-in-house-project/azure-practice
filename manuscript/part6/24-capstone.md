...
データプレーンのロールはどうでしょうか。Cosmos は独自の仕組みを持っていましたが、Service Bus は ARM（Azure Resource Manager）の RBAC の中にあります。
...
型に沿って問いを置いていくと、未習のサービスでも「何を確認すれば設計に組み込めるか」が最初から見えています。これが、サービスを 6 つに絞って同じ型を繰り返した理由です。残りのサービスへの適用は、付録 A のワークシートが受け持ちます。

## ここまでで出来上がったもの

未習のサービスを型で読む練習として作ったものをまとめます。

```mermaid
flowchart TB
  SUB["サブスクリプション (課金の境界)"] --> RP["Microsoft.ServiceBus (リソースプロバイダー、Registered)"]
  SUB --> RG["azp-ch24-rg (リソースグループ)"]
  RG --> NS["Service Bus 名前空間 (Basic)"]
  NS --> Q["キュー (メッセージの置き場)"]
  NS -.->|"キー認証の代わりに使う"| MI["マネージド ID とデータのロール"]
```

見どころは、初めて触るサービスでも確かめる順序が同じことです。プロバイダーを登録し、リソースグループに置き、料金の段階を選び、キーの代わりに ID で通す道があるかを調べました。第 4 部の 6 ブロックの型は、本書が扱わなかったサービスにもそのまま使えます。
