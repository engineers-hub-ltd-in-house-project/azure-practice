...
呼び出し側が次の一手（ロール割り当てなど）に必要とする値だけを outputs で返します。中のリソースへ直接触らせず、入口と出口だけでやり取りします。関数と同じ設計です。
...
```bash
az deployment group create -g azp-ch12-rg \
  --template-file infra/bicep/chapters/ch12-modules/main.bicep
```

応答から outputs と state だけを抜き出したものです。
...
| 状況                                       | 選択                                     |
| ------------------------------------------ | ---------------------------------------- |
| AVM に該当モジュールがあり、要件を満たす   | AVM を使う。同じものを作り直さない       |
...
26 行のテンプレートから何ができたのかをまとめます。

```mermaid
flowchart TB
  MAIN["main.bicep (26 行)"] --> M1["keyless-storage.bicep (自作モジュール)"]
  MAIN --> M2["AVM のユーザー割り当てマネージド ID モジュール"]
  M1 --> ST["ストレージアカウント (キー認証は無効)"]
  M2 --> ID["azp-ch12-id (ユーザー割り当てマネージド ID)"]
  RG["azp-ch12-rg (リソースグループ)"] --> ST
  RG --> ID
```

見どころは、呼び出し側の行数と、出来上がるリソースの量が釣り合わないことです。26 行のテンプレートが 2 つのモジュールを呼び、モジュールの中身がリソースの詳細を引き受けています。自作モジュールは相対パスで、AVM モジュールは公式のレジストリからバージョンを指定して取得します。どこから取得するかが違うだけで、呼び出し側の書き方は同じです。
