...
第 1 部の 4 階層のうち、テンプレートを差し込める場所は 4 つあり、それぞれに対応するコマンドがあります。
...
deleteAll はスタックの削除と同時に管理下のリソースをすべて消します。リソースグループの削除（第 3 章）と同じ「まとめて消える」動きを、リソースグループとは独立した単位で持てるということです。実際、1 つのリソースグループに複数のスタックを置いて、ライフサイクルをリソースグループより細かく切ることもできます。
