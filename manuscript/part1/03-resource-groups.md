...
次の図では、リソースの例として 2 つのサービスの名前を出します。Function App はコードを動かすためのリソースで第14章、Key Vault はパスワードや証明書といった秘密の情報を預けるための保管庫で第16章で扱います。図での役どころは、リソースグループを跨いで参照し合う 2 つのリソースです。Key Vault はこの章の後半でもう一度出てきます。
