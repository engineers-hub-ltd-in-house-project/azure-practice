...
1 つ目はストレージアカウントです。ファイルやデータを置くためのリソースで、Azure でもっともよく使うものの 1 つです。中身の仕組みは第 15 章で扱うので、ここでは「リソースグループの中に置かれる、データの置き場」とだけ捉えてください。

2 つ目はユーザー割り当てマネージド ID です。第 3 章のクリーンアップ演習で作ったのと同じ種類のもので、リソースとしての側面と、テナントの台帳の 1 行という側面の 2 つを持ちます。第 3 章のものは片付け済みなので、この章専用のものを新しく作ります。
...
一覧はテナントルートに対する読み取りを使うため、いま作った管理グループが即座には現れないことがあります。本書の検証でも、`show` では見えるのに `list` には出ない状態を観測しました。作られたかどうかは、一覧ではなく `show` の結果で判断してください。
...
4. ストレージアカウントを開き、左のメニューから「アクセス制御 (IAM)」を選びます。開いた直後は「アクセスの確認」タブが選ばれていて、ロールの割り当ては表示されません。上部のタブを「ロールの割り当て」に切り替えます
5. 一覧の「スコープ」列を見ます。行ごとに、その割り当てがどのスコープで行われたものかが書かれています

本書の検証環境では、この一覧はこうなりました。日本語のポータルでの表示です。

| 名前        | 種類          | 役割                             | スコープ                      |
| ----------- | ------------- | -------------------------------- | ----------------------------- |
| （あなた）  | ユーザー      | 所有者                           | サブスクリプション (継承済み) |
| （あなた）  | ユーザー      | 所有者                           | 管理グループ (継承済み)       |
| azp-ch04-id | マネージド ID | ストレージ BLOB データ共同作成者 | このリソース                  |

読み取れることが 2 つあります。

1 つ目は表記です。CLI で `Storage Blob Data Contributor` と指定したロールが、ポータルでは「ストレージ BLOB データ共同作成者」と表示されます。同じものの別表記です。CLI とポータルを行き来するときは、この 2 つを同じロールとして読んでください。

2 つ目が本題です。マネージド ID の行だけがスコープ「このリソース」で、所有者の行は「継承済み」と付いています。第 2 章で `az role assignment list` に `--include-inherited` を付けたときに増えた行が、ポータルではこの「継承済み」の行にあたります。ポータルは既定で継承されている割り当ても混ぜて表示し、どこから来たかをスコープ列で示す作りです。

「管理グループ (継承済み)」の行は、この章の手順が作ったものです。ステップ 1 で管理グループを作ったときに、作成した本人へ所有者の割り当てが付きました。ステップ 2 でサブスクリプションをその管理グループの下へ移したことで、その割り当てがサブスクリプションを通ってこのストレージアカウントまで届いています。第 2 章で図にした継承が、実物の一覧に現れた形です。

なお本書の検証では、所有者の行がもう 1 つ（サブスクリプションからの継承）重複して現れました。同じ ID に同じロールが別々の経路で割り当たると、行が分かれて表示されます。
...
片付ける前に、4 階層と、そこに置いたものの関係を確かめます。

```mermaid
flowchart TB
  MG["azp-ch04-mg (管理グループ)"] --> SUB["サブスクリプション (課金の境界)"]
  SUB --> RG["azp-ch04-rg (リソースグループ)"]
  RG --> ST["ストレージアカウント (キー認証は無効)"]
  RG --> ID["azp-ch04-id (ユーザー割り当てマネージド ID)"]
  ID -->|"Storage Blob Data Contributor"| ST
  SUB -.->|"スコープ違反で作れなかった"| NG["サブスクリプションに直接置こうとしたストレージ"]
```

見どころは 2 つです。1 つ目は、ロール割り当ての線だけが階層の並びからはみ出していることです。スコープはストレージアカウント 1 つで、リソースグループでもサブスクリプションでもありません。2 つ目は、壊す演習で作れなかったものが階層のどこにも入っていないことです。リソースを置ける場所はリソースグループだけで、サブスクリプションに直接は置けません。次の片付けは、この図を下から上へ辿る順序になります。

## クリーンアップ演習

第 3 章で teardown と呼ぶことにした後片付けを、この章の構成に対して行います。作ったものが 4 階層にまたがるぶん、順序の制約がはっきり出ます。

```bash
./scripts/teardown/ch04-foundation.sh
```

削除は作成の逆順ではありません。依存されている側は、依存している側より後にしか消せません。この teardown は次の 5 手で進みます。
