...
| サービスプリンシパル                  | 人ではなくプログラムのための ID。デプロイの自動化やアプリケーションがこれで認証します                                                      | 第5章         |
...
| リソース                              | 仮想マシン、ストレージ、データベースなど、実際に動くもの 1 つ 1 つ                                                                         | 第14〜19章    |
| リソースプロバイダー                  | リソースの種類ごとの提供元。サブスクリプションごとに有効・無効の状態を持ちます                                                             | 第1章         |
| リージョン                            | リソースが物理的に置かれる場所（japaneast など）。リソースごとに選びます                                                                   | 第3章         |
| ロール割り当て                        | 誰が・どの範囲で・何をできるかの指定。ID の世界とリソースの世界を繋ぐ橋です。この仕組み全体を RBAC（ロールベースのアクセス制御）と呼びます | 第6章         |
| 課金アカウント                        | 支払い方法と請求書の単位。サブスクリプションより上位の契約情報です                                                                         | 第23章        |
| Azure Resource Manager                | Azure への管理操作をすべて受け付ける窓口。ポータルも CLI も、最終的にはここへ同じ形式の指示を送ります。ARM と略されます                    | 第1章・第10章 |
| Bicep                                 | Azure のリソース構成をコードで書くための言語。ARM が受け付ける JSON の書きやすい表記で、変換してから送られます                             | 第10章        |
