...
Azure の入門書の多くは、アカウントが既にある前提で始まります。しかし実際には、サインアップの過程でいくつもの要素（アカウント、テナント、サブスクリプション）が作られており、ここを素通りすると「自分がいま何を作ったのか」が分からないまま先へ進むことになります。
...
| リソースグループ                      | リソースを入れる箱。まとめて消せることが最大の特徴です                                                                                     | 第 3 章           |
...
| ロール割り当て                        | 誰が・どの範囲で・何をできるかの指定。ID の世界とリソースの世界を繋ぐ橋です。この仕組み全体を RBAC（ロールベースのアクセス制御）と呼びます | 第 6 章           |
