...
管理グループとは、サブスクリプションと他の管理グループを入れられる、入れ子にできる箱です。箱自体はリソースを持てず、名前と親子関係のほかには何も持ちません。ポリシーとロール割り当ての掛け先になることだけが仕事の、統制のためだけの軽い入れ物です。入れ子にできるので、階層は複数段（最大 6 段）に組めます。

ただの箱なのに章を割く理由は、これが「サブスクリプションのフォルダ」ではなく継承の経路だからです。フォルダは中身を整理するだけですが、管理グループは上に置いたものを配下のすべてに継承させます。この違いが、階層設計をあとから動かしにくくしている原因でもあります。
...
次の図は、実務で広く使われている階層の例です。図中の platform は全社共通の基盤を置く枝（配下の connectivity はネットワーク基盤、identity は ID 基盤）、landingzones は実際のワークロードが載るサブスクリプションを置く枝で、本番（prod）と開発（dev）で統制の強さを分けています。この切り方の意味は本章の後半と第21章で扱うので、ここでは「ルートの下に目的別の枝が生える」という形だけ掴んでください。
