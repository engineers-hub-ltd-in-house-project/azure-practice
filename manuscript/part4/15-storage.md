...
第 14 章で Function App を作ったとき、コマンドに `--storage-account` を渡しました。Functions はストレージなしでは作れません。本章はこの隠れた依存先の側から Storage を理解し、最後にその依存関係を完全キーレスに作り替えます。
...
| リソースグループ   | ライフサイクル境界。第 14 章の teardown で依存先ごと消えたのはこの設計です |
...
RBAC のスコープ選択について毎章の確認です。ここでのコンテナーは、実行環境のコンテナー（第 7 章）ではなく、BLOB をまとめるストレージ側の入れ物で、本章では BLOB コンテナーと書きます。データプレーンのロールは、アカウント単位だけでなく BLOB コンテナー単位のスコープにも割り当てられます。「この BLOB コンテナーだけ読ませたい」はロール割り当てで表現でき、ガバナンス階層との一致は要りません。
...
ネットワーク面では、既定でパブリックなエンドポイントを持ちます。本書のハンズオンでは `--allow-blob-public-access false` で匿名アクセスを塞いでいますが、これは「認証なしの公開」を塞ぐだけで、エンドポイント自体はインターネットに面したままです。閉域化（Private Endpoint）は第 19 章で扱います。
...
手順の実体は `scripts/chapters/ch15-storage.sh` にあります。Function App を作り、その依存先のストレージを完全キーレス化するところまでを 1 本で実行します。
...
Functions のデプロイパッケージ（コードの zip）の置き場です。ほかにも、タイマーやキューのトリガーを使えば実行状態の管理に BLOB やキューが使われます。Functions にとってストレージは、コードと実行状態の置き場です。この置き場を外部に持つからこそ、Functions 自体は状態を抱えずに済みます。
...
第 14 章と同じくリソースグループ 1 つの削除です。Function App・プラン・Application Insights・ストレージ・ロール割り当てが一括で消えます。
...
1. Function App とその依存先のストレージを別々のリソースグループに置く設計には、どんな問題が起きますか。第 3 章の原則と、本章の teardown の挙動から答えてください
