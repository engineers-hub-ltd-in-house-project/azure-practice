...
第 14 章で Function App を作ったとき、コマンドに `--storage-account` を渡しました。Functions はストレージなしでは作れません。本章はこの「隠れた相棒」の側から Storage を理解し、最後に相棒関係を完全キーレスに作り替えます。
...
| リソースグループ   | ライフサイクル境界。第 14 章の teardown で相棒ごと消えたのはこの設計です |
...
1. Function App とその相棒ストレージを別々のリソースグループに置く設計には、どんな問題が生まれますか。第 3 章の原則と、本章の teardown の挙動から答えてください
