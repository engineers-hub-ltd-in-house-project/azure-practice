...
Cosmos DB は Azure のグローバル分散データベースです。本章の主題はデータベースの機能ではなく、その認可の構造にあります。Cosmos DB のデータプレーン RBAC は、ここまで学んだ ARM（Azure Resource Manager）の RBAC と名前も概念もよく似た、しかし完全に別の仕組みです。ポータルの IAM 画面からは設定できず、専用のロール定義と割り当てを持ちます。
...
| 管理グループ       | 継承されるポリシーと RBAC。第 20 章で disableLocalAuth を強制します  |
...
Cosmos DB も、作成した時点でアクセスキー（マスターキー）を持っています。第 8 章の流れの最終形として、キー認証そのものを止める設定が disableLocalAuth です。
...
キー認証を無効にしたアカウントが、どういう形になったかをまとめます。

```mermaid
flowchart TB
  RG["azp-ch18-rg (リソースグループ)"] --> ACC["azp-ch18-cosmos (アカウント、disableLocalAuth)"]
  ACC --> DB["demo (データベース)"]
  ME["自分 (ユーザー)"] -->|"Cosmos DB のデータプレーンのロール割り当て"| ACC
  ME -.->|"ARM のロールでは中身に触れない"| ACC
```

見どころは、ロール割り当ての系統が 2 つあることです。ポータルの IAM 画面に出るのは ARM 側の割り当てで、データの読み書きを決めるのは `az cosmosdb sql role assignment` で作る別の系統の割り当てです。監査でアクセスできる相手を数えるときは、この 2 つを別々に一覧する必要があります。
...
2. disableLocalAuth を有効にした直後、接続していたアプリケーション（第 5 章）が 401 で停止しました。エラーメッセージに含まれるはずの文言と、復旧に必要な 2 つの設定（認証方式の変更とロールの割り当て）を、本章のコマンドで説明してください
