...
Azure の学習でつまずく最大の原因は、個々のサービスの使い方を先に覚えてしまい、サブスクリプションやリソースグループといった土台の構造をあとから継ぎ足すことにある。本書は逆順を取る。最初にガバナンス階層を体に入れ、そのあとで個々のサービスを、その階層とどう繋がっているかという **縦の関係** と、認証認可・ネットワーク・課金とどう繋がっているかという **横の関係** の両方から学ぶ。
...
他クラウドの用語に置き換えて理解する方法（対応表による学習）は使わない。Azure 独自の語彙と構造だけで理解が完結するように書く。

## 本書の核 ― 6 ブロックの型

Part 4 の各章は、以下の 6 ブロックで統一して書かれている。サービスを 6 つに絞って型を深く固定し、扱わなかったサービスへは読者自身が型を適用して広げる設計になっている（付録 A と第 24 章がその受け皿）。
...
| 4   | 横の繋がり（インフラ足回りとネットワーク） | ホスティングプランや SKU の違いがネットワーク機能・可用性にどう波及するか                                      |
| 5   | 横の繋がり（契約・課金）                   | 従量課金か専有課金か。課金・無料枠がどの階層で集計されるか                                                     |
| 6   | ハンズオン                                 | 壊す演習とクリーンアップ演習を必ず含む                                                                         |

この型は散文の申し合わせではなく、`scripts/lint/structure-lint.mjs` が CI で機械的に検査している。

## Operation as Code

本文に載るコマンドと Bicep は、すべてリポジトリ内の実ファイルとして存在する。本文は説明を担い、実行可能な手順は `scripts/` と `infra/` を参照する。読者がコピー&ペーストする対象と、CI が検証する対象を同一にするための構成である。
...
読む順序は番号どおり。Part 1 を飛ばすと本書は成立しない。
...
| 章  | タイトル                                                                                                            | 状態 |
| --- | ------------------------------------------------------------------------------------------------------------------- | ---- |
| 5   | [Entra ID の基本 ― ユーザー・グループ・サービスプリンシパル・マネージド ID](manuscript/part2/05-entra-id-basics.md) | 骨格 |
| 6   | [RBAC ― スコープとロールの掛け算で権限が決まる](manuscript/part2/06-rbac.md)                                        | 骨格 |
| 7   | [マネージド ID ― リソース間の内向き認証](manuscript/part2/07-managed-identity.md)                                   | 骨格 |
...
| 章  | タイトル                                                                                    | 状態 |
| --- | ------------------------------------------------------------------------------------------- | ---- |
| 10  | [ARM テンプレートと Bicep の関係](manuscript/part3/10-arm-and-bicep.md)                     | 骨格 |
| 11  | [デプロイスコープと Deployment Stacks](manuscript/part3/11-deployment-scopes-and-stacks.md) | 骨格 |
| 12  | [モジュール化と Azure Verified Modules](manuscript/part3/12-modules-and-avm.md)             | 骨格 |
...
| 章  | タイトル                                                                                              | 状態 |
| --- | ----------------------------------------------------------------------------------------------------- | ---- |
| 14  | [Functions ― Flex Consumption を軸に課金とネットワークの関係を辿る](manuscript/part4/14-functions.md) | 骨格 |
| 15  | [Storage ― Functions の隠れた依存関係から理解する](manuscript/part4/15-storage.md)                    | 骨格 |
...
- Azure サブスクリプション。ハンズオンで扱うスコープ上、**サブスクリプションに対する Owner** が必要（第 6・7・9 章で RBAC ロール割り当てを行うため、Contributor では不足する）
- 第 2・4 章の管理グループ操作には、テナントルートでの昇格または Management Group Contributor が必要
- Azure CLI と Bicep CLI。バージョンは `scripts/bootstrap/check-env.sh` が確認する

**顧客環境や本番サブスクリプションでは絶対に実行しないこと。** 本書のハンズオンはリソースの作成・削除・ポリシー割り当てを含む。
...
無料枠と低額 SKU で完結する構成を前提にしている。ただし以下は課金される。章の冒頭に概算を明記している。

| 章                     | 課金                                                         |
| ---------------------- | ------------------------------------------------------------ |
| 17（AKS）              | ノードプールの VM が時間課金。既定では実行しない章として扱う |
| 19（Private Endpoint） | エンドポイント 1 つあたり時間課金 + データ処理量             |
| 14〜16, 18, 20         | 低額。章末の teardown を必ず実行すること                     |

各章の teardown スクリプトは、その章で作ったものを削除する。Key Vault のように論理削除（soft-delete）されるリソースは `purge` まで行う。詳細は第 3 章。

## 執筆と検証

執筆規約は [WRITING_GUIDELINES.md](WRITING_GUIDELINES.md) にある。検証していない手順は本文に書かない。各章には検証時の CLI バージョンと API バージョンが記録される。
...
現在の検証状況は [docs/verification-log.md](docs/verification-log.md) を参照。
